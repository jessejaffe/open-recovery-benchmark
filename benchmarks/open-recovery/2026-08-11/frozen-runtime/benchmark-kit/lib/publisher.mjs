...
function caseMarkup(item) {
  const checks = Object.entries(item.validation.checks)
    ...
    .join("");
  const returnedFiles = item.observation.returnedFiles ?? [];
  const evidence = [...new Set([
    item.observation.stdout,
    item.observation.stderr,
    item.observation.output?.path,
    ...returnedFiles.map((returnedFile) => returnedFile.path),
  ...
  const returnedMarkup = returnedFiles.length === 0 ? "" : `<p class="evidence"><b>Returned files:</b> ${returnedFiles.map((returnedFile) => (
    `<code>${escapeHtml(returnedFile.path)}</code> — ${escapeHtml(returnedFile.classification)}`
  )).join(" · ")}</p>`;
  const selectionMarkup = item.observation.outputSelection
    ? `<p class="evidence"><b>Selection:</b> ${escapeHtml(item.observation.outputSelection.strategy)}</p>`
    : "";
  return `<details class="case"><summary><span>${escapeHtml(item.caseId)}</span><strong class="${escapeHtml(item.disposition)}">${escapeHtml(item.disposition)}</strong></summary><div class="case-body"><p><b>Observed:</b> ${escapeHtml(item.observation.disposition)} · <b>Independent validator:</b> ${escapeHtml(item.validation.validator)}</p><ul>${checks}</ul>${selectionMarkup}${returnedMarkup}<p class="evidence">Evidence: ${evidenceMarkup}</p></div></details>`;
}
...
function renderHtml(run, attestation, plan, protocol) {
  ...
  const qualification = run.summary.competitiveSummaryEligible
    ...
    : "Synthetic qualification — excluded from competitor performance summaries";
  return `<!doctype html>
...
:root{color-scheme:light;--ink:#162724;--muted:#60706b;--paper:#f4f0e8;--card:#fffdf8;--green:#164f43;--mint:#cfe6dc;--coral:#b94f38;--line:#cfc9bc}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 88% 4%,#cfe6dc 0,transparent 28rem),var(--paper);color:var(--ink);font:16px/1.55 system-ui,sans-serif}main{width:min(980px,calc(100% - 32px));margin:auto;padding:64px 0 90px}.eyebrow{color:var(--green);font-size:12px;font-weight:800;letter-spacing:.14em;text-transform:uppercase}h1{max-width:820px;margin:18px 0 16px;font:700 clamp(42px,7vw,78px)/.96 Georgia,serif;letter-spacing:-.045em}.lede{max-width:760px;color:var(--muted);font-size:19px}.boundary{margin:32px 0;padding:18px 20px;border-left:4px solid #d88a3d;background:#fff8e8}.identity{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:30px 0}.identity article,.digest{padding:20px;border:1px solid var(--line);border-radius:12px;background:rgba(255,253,248,.8)}.identity h2{margin:0 0 12px;font-size:18px}.identity p{margin:7px 0;color:var(--muted)}.qualification{color:var(--green)!important;font-weight:750}.digest{display:grid;grid-template-columns:1fr;gap:10px;margin:32px 0}code{font:12px/1.5 ui-monospace,monospace;overflow-wrap:anywhere}.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:30px 0}.stat{padding:18px;border:1px solid var(--line);border-radius:12px;background:var(--card)}.stat strong{display:block;font-size:30px}.stat span{color:var(--muted);font-size:12px}.limitations{margin:30px 0;padding:20px 24px;border:1px solid var(--line);border-radius:12px;background:var(--card)}.case{margin:12px 0;border:1px solid var(--line);border-radius:12px;background:var(--card);overflow:hidden}.case summary{display:flex;justify-content:space-between;gap:16px;padding:18px 20px;cursor:pointer;font-weight:700}.case summary strong{font-size:12px;text-transform:uppercase}.verified-pass{color:var(--green)}.changed-output,.refusal{color:var(--coral)}.case-body{padding:0 20px 20px;border-top:1px solid var(--line)}.case-body ul{padding:0;list-style:none}.case-body li{display:flex;justify-content:space-between;gap:20px;padding:7px 0;border-bottom:1px solid #ebe6dc}.evidence{color:var(--muted)}footer{margin-top:36px;color:var(--muted)}@media(max-width:700px){.identity,.stats{grid-template-columns:1fr}.case summary{align-items:flex-start;flex-direction:column}}
</style></head><body><main><p class="eyebrow">Open recovery benchmark · independently verified</p><h1>${escapeHtml(run.protocol.title)}</h1><p class="lede">${escapeHtml(run.protocol.publicClaim)}</p><div class="boundary"><strong>Claim boundary</strong><br>${escapeHtml(run.claimBoundary)}</div><section class="identity"><article><h2>Tool under test</h2><p><b>${escapeHtml(run.tool.vendor)} · ${escapeHtml(run.tool.product)}</b></p><p>Version ${escapeHtml(run.tool.version)} · ${escapeHtml(run.tool.platform)}</p><p class="qualification">${escapeHtml(qualification)}</p></article><article><h2>Frozen corpus</h2><p><b>${escapeHtml(run.corpus.title)}</b></p><p>ID <code>${escapeHtml(run.corpus.id)}</code></p><p>Digest <code>${escapeHtml(run.corpus.digest)}</code></p></article><article><h2>Frozen protocol</h2><p>ID <code>${escapeHtml(run.protocol.id)}</code> · frozen ${escapeHtml(protocol.frozenAt)}</p><p>Digest <code>${escapeHtml(run.protocol.digest)}</code></p><p>Score: ${escapeHtml(plan.scoring.passDisposition)} over ${escapeHtml(plan.scoring.denominator)}</p></article><article><h2>Execution limits</h2><p>${escapeHtml(plan.limits.timeoutMs)} ms per case timeout</p><p>${escapeHtml(plan.limits.maxCapturedBytes)} bytes maximum captured stdout/stderr</p><p>Harness ${escapeHtml(plan.harness.name)} ${escapeHtml(plan.harness.version)}</p></article></section><section class="digest"><div><b>Frozen plan</b><br><code>${escapeHtml(run.planDigest)}</code></div><div><b>Evidence root</b><br><code>${escapeHtml(attestation.rootDigest)}</code></div><div><b>Signature</b> ${escapeHtml(attestation.signatureStatus)}</div></section><section class="stats"><div class="stat"><strong>${run.summary.eligibleCases}</strong><span>eligible cases</span></div><div class="stat"><strong>${run.summary.verifiedPasses}</strong><span>verified passes</span></div><div class="stat"><strong>${run.summary.changedOutputs}</strong><span>changed outputs</span></div><div class="stat"><strong>${run.summary.refusals}</strong><span>refusals</span></div><div class="stat"><strong>${run.summary.unavailable}</strong><span>unavailable or paywalled</span></div><div class="stat"><strong>${run.summary.errors}</strong><span>errors, timeouts, or no output</span></div></section><section class="limitations"><h2>Claim limitations</h2><p>${escapeHtml(run.claimBoundary)}</p><ul>${limitations}</ul></section><h2>Case evidence</h2>${cases}<footer>Verify the complete publication locally: <code>node benchmark-kit/cli.mjs verify-publication --publication .</code>. Product observations never determine a pass; the benchmark-owned validator does.</footer></main></body></html>`;
}
