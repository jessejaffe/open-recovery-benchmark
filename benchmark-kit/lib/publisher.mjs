...
function caseMarkup(item) {
  ...
  return `<details class="case"><summary><span>${escapeHtml(item.caseId)}</span><strong class="${escapeHtml(item.disposition)}">${escapeHtml(item.disposition)}</strong></summary><div class="case-body"><p><b>Category:</b> ${escapeHtml(item.category)} · <b>Recovery score:</b> ${escapeHtml(item.score)}</p><p><b>Observed:</b> ${escapeHtml(item.observation.disposition)} · <b>Independent validator:</b> ${escapeHtml(item.validation.validator)}</p><ul>${checks}</ul><p class="evidence">Evidence: ${evidenceMarkup}</p></div></details>`;
}
